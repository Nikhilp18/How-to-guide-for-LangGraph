...
def read_texts_and_pdfs(folder_path):
    combined_text = ""
        ...
        if os.path.isfile(file_path):
            if filename.lower().endswith('.txt'):
                ...
                try:
                    ...
                    for page in reader.pages:
                        combined_text += page.extract_text() or ""  # extract_text might return None
                    combined_text += "\n"
                except Exception as e:
...
def emded_documents(state: State) -> State: 
    ...
    persist_directory = config.get("Embedding_model_config", "PERSIST_DIRECTORY")
    corpus_doc = read_texts_and_pdfs(doc_path)
...
def generate(state: State):
    ...
    print("RAG-bot:", state["messages"][-1].content)
...
def main():
    ...
    graph.add_edge("emded_documents", "user_speak")
    ...
    graph.add_edge("generate", "user_speak")
    ...
    with open("RAG-based_chat_bot.png", "wb") as f:
        f.write(image_data)
